...
window.ACS_TEXT_TO_MODEL_PROVIDERS = [
  ...
  {
    id: 'nvidia-nim-trellis',
    label: 'NVIDIA NIM TRELLIS',
    endpoint: 'POST {backendURL}/v1/...',
    inputs: ['imageUrl'],
    output: 'glb',
  },
];
...
window.ACS_MODEL_PRESETS = [
  ...
  },
  {
    ...
    credit: '© DWANGO',
    license: 'Niconi Commons Attribution',
  },
  {
    id: 'seed',
    label: 'Seed-san',
  ...
  {
    id: 'vsekai',
    label: 'three-vrm girl 1.0β',
    url: '…',
    format: 'vrm',
    kind: 'humanoid',
    credit: 'V-Sekai community',
...
window.ACS_loadModelFromURL = async function (url, opts) {
  ...
  const buf = await fetch(ACS_normalizeModelURL(url)).then((r) =>
    r.arrayBuffer()
  );
  switch (fmt) {
    case 'vrm':
      return loadVRMBuffer(buf, url, opts); // existing path
    case 'glb':
      return loadGLBBuffer(buf, url, opts); // new (no VRMLoaderPlugin)
    case 'fbx':
      return loadFBXBuffer(buf, url, opts);
    case 'ply':
      return loadPLYBuffer(buf, url, opts);
    case 'obj':
      return loadOBJBuffer(buf, url, opts);
    case 'mjcf':
      return ACS_loadGearSonicRobotModel(opts.THREE, { sceneURL: url });
  }
};
```
...
| Some FBX files look like animations, not models — backwards compat risk. | Keep the existing `S.Section "Animation"` FBX path. The model selector only treats an FBX as a _model_ when no VRM is currently loaded (preserves drag-drop semantic). |
